...
HudModule::HudModule() : IModule(0, Category::GUI, "Displays ArrayList/TabGUI") {
	...
	registerFloatSetting("FPS X", &fpspos2, fpspos2, 1.f, 450.f);
	registerFloatSetting("FPS Y", &fpspos, fpspos, 1.f, 150.f);
	registerFloatSetting("CPS X", &cpspos2, cpspos2, 1.f, 450.f);
	registerFloatSetting("CPS Y", &cpspos, cpspos, 1.f, 150.f);
...
void HudModule::onPostRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	{  // Coordinates
		if (!(g_Data.getLocalPlayer() == nullptr || !this->coordinates)) {
			...
			vec2_t textPos = vec2_t(rectPos.x + 3.5f, rectPos.y + 1.f);
