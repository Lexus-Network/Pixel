...
SetPrefixCommand::SetPrefixCommand() : IMCCommand("setprefix", "Set the prefix for Pixel commands", "<prefix>") {
...
bool SetPrefixCommand::execute(std::vector<std::string>* args) {
	...
	clientMessageF("[%sPixel%s] %sSet prefix to %s%c", GOLD, WHITE, GREEN, GRAY, prefix);
	return true;
}
