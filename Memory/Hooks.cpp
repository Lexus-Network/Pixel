...
void Hooks::ClientInstanceScreenModel_sendChatMessage(void* _this, TextHolder* text) {
	...
	if (text->getTextLength() > 0) {
		...
		if (*message == cmdMgr->prefix) {
			...
			if (!helpedUser) {
				helpedUser = true;
				g_Data.getClientInstance()->getGuiData()->displayClientMessageF("%sYour Pixel prefix is: \"%s%c%s\"", RED, YELLOW, cmdMgr->prefix, RED);
			...
			if (strcmp(screenName.c_str(), "start_screen") == 0) {
...

				{
					...
					if (shouldRenderWatermark) {
						...
						static const float textHeight = (nameTextSize + versionTextSize * 0.4f /* We don't quite want the version string in its own line, just a bit below the name */) * DrawUtils::getFont(Fonts::MCFONT)->getLineHeight();
						constexpr float borderPadding = 1;
						constexpr float margin = 5;
						//auto player = g_Data.getLocalPlayer();
						//std::string playername = player->getNameTag()->getText();

						static std::string name = " Pixel Client";	//+ playername;
#ifdef _DEBUG			
						...
						DrawUtils::fillRectangle(vec4_t(windowSize.x / windowSize.x, windowSize.y / windowSize.y, +nameLength + 5, 12), MC_Color(16, 16, 16), .4);
						DrawUtils::fillRectangle(vec4_t(windowSize.x / windowSize.x, windowSize.y / windowSize.y, +nameLength + 5, 2), MC_Color(currColor), 1.f);
...
void prepCoolBean() {
	if (g_Data.getClientInstance() && g_Data.getClientInstance()->minecraftGame->getServerEntries() && *reinterpret_cast<__int64*>(g_Data.getClientInstance()->minecraftGame->getServerEntries() + 0x50)) {
...

		{
			...
			t->serverName.setText("Pixel Server");
			t->serverName2.setText("Pixel Server");  // This is the one actually displayed
